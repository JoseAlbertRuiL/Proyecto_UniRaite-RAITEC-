...
const ConducirScreen = ({ navigation }: any) => {
  ...
  const obtenerDatosConducir = async () => {
    ...
    if (activosData.success) {
      ...
      if (expirados.length > 0) {
        Promise.allSettled(
          ...
          const cancelados = results.filter(
            (r) => r.status === "fulfilled"
          ).length;

          if (cancelados > 0) {
            console.log(
              `🗑️ ${cancelados} viaje(s) expirado(s) cancelado(s) automáticamente`
            );

            queryClient.invalidateQueries({
              queryKey: ["conducir-datos"],
            });
          }
        });
      }

      viajesActivos = todosActivos.filter((viaje: any) => {
        const haIniciado = viaje.viajes_activos?.length > 0;
        const fechaPasada = new Date(viaje.fecha_hora_salida) < ahora;
        return haIniciado || !fechaPasada;
      });
    ...
    return {
      viajesActivos,
      solicitudes: solicitudesData.success
  ...
  const {
    ...
    queryKey: ["conducir-datos"],
    queryFn: obtenerDatosConducir,
  });

  const viajesActivos = data?.viajesActivos ?? [];
  const solicitudes = data?.solicitudes ?? [];

  const esViajeActivo = (idViajePub: number) => {
    if (!Array.isArray(viajesActivos) || viajesActivos.length === 0) {
      return false;
    }

    const viajeActual = viajesActivos.find(
      (v: any) => v.id_viaje_pub === idViajePub
    );
  ...
  const handleIniciarViaje = async (viajeId: number) => {
    ...
    try {
      ...
      if (result.success) {
        const viajeCompleto = viajesActivos.find(
          (v: any) => v.id_viaje_pub === viajeId
        );
        ...
        Alert.alert(
          "¡Viaje iniciado!",
          "Compartiendo tu ubicación con los pasajeros."
        );
  ...
  const iniciarTransmisionUbicacion = async (
    viajeId: number,
    ...
    if (status !== "granted") {
      Alert.alert(
        "Permiso requerido",
        "Necesitas dar permiso de ubicación para transmitir."
      );
      return;
    }
    ...
    >
      <View className="flex-row justify-between items-start">
        <View className="flex-1">
          ...
          <Text className="text-gray-800 font-medium">{viaje.origen_texto}</Text>
          <Text className="text-sm text-red-600 font-semibold mt-2">DESTINO</Text>
          <Text className="text-gray-800 font-medium">{viaje.destino_texto}</Text>
        </View>
      ...
      if (viajesActivos.length === 0) {
        return (
          <View className="flex-1 items-center justify-center py-20">
            ...
            >
              <Text className="text-white font-semibold">Publicar un viaje</Text>
            </TouchableOpacity>
          </View>
        );
      }
  ...
  return (
    <ScreenWrapper hasFooter={true}>
      ...
      <View className="flex-row border-b border-gray-200">
        <TouchableOpacity
          className={`flex-1 py-3 items-center ${
            activeTab === "activos" ? "border-b-2 border-blue-900" : ""
          }`}
          onPress={() => setActiveTab("activos")}
        >
          <Text
            className={`font-semibold ${
              activeTab === "activos" ? "text-blue-900" : "text-gray-500"
            }`}
          >
            Activos
          </Text>
        ...
        <TouchableOpacity
          className={`flex-1 py-3 items-center ${
            activeTab === "solicitudes" ? "border-b-2 border-blue-900" : ""
          }`}
          onPress={() => setActiveTab("solicitudes")}
        >
          <Text
            className={`font-semibold ${
              activeTab === "solicitudes" ? "text-blue-900" : "text-gray-500"
            }`}
          >
            Solicitudes
          </Text>
      ...
      {liveMapViajeId &&
        (() => {
          const viaje = viajesActivos.find(
            (v: any) => v.id_viaje_pub === liveMapViajeId
          );
